...
TEST(Variant, Void) {
    ...
    EXPECT_THROW((void)variant.Get<int>(), std::logic_error);
    EXPECT_THROW((void)variant.Get<const int>(), std::logic_error);
    EXPECT_THROW((void)variant.Get<int&>(), std::logic_error);
    EXPECT_THROW((void)variant.Get<const int&>(), std::logic_error);
...
TEST(Variant, Value) {
    ...
    EXPECT_EQ(variant.Get<const int>(), 42);
    EXPECT_EQ(variant.Get<int&>(), 42);
    EXPECT_EQ(variant.Get<const int&>(), 42);

    // Modifying through reference should modify the original
    variant.Get<int&>() = 100;
    EXPECT_EQ(variant.Get<int>(), 100);
}
...
TEST(Variant, Pointer) {
    ...
    EXPECT_EQ(variant.Get<int*>(), &value);
    EXPECT_EQ(variant.Get<const int*>(), &value);
...
TEST(Variant, CustomClassGet) {
    ...
    EXPECT_EQ(variant.Get<TestStruct>().value, 42);
    EXPECT_EQ(variant.Get<const TestStruct>().value, 42);
    EXPECT_EQ(variant.Get<TestStruct&>().value, 42);
    EXPECT_EQ(variant.Get<const TestStruct&>().value, 42);

    // Modifying through reference should modify the variant's copy
    variant.Get<TestStruct&>().value = 100;
    EXPECT_EQ(variant.Get<TestStruct>().value, 100);
}
...
TEST(Variant, TypeMismatch) {
    ...
    EXPECT_THROW((void)variant.Get<double>(), std::logic_error);
    EXPECT_THROW((void)variant.Get<std::string>(), std::logic_error);
    EXPECT_THROW((void)variant.Get<TestStruct>(), std::logic_error);
}
...
TEST(Variant, ComplexTypeConversions) {
    ...
    const auto v4 = CreateVariant<TestStruct&&>(static_cast<TestStruct&&>(obj)); // rvalue

    // Check value copy
    EXPECT_TRUE(v1.CanGet<TestStruct>());
    EXPECT_EQ(v1.Get<TestStruct>().value, 42);

    // Check reference wrapper
    EXPECT_TRUE(v2.CanGet<TestStruct&>());
    EXPECT_EQ(v2.Get<TestStruct&>().value, 42);

    // Check pointer
    EXPECT_TRUE(v3.CanGet<TestStruct*>());
    EXPECT_EQ(v3.Get<TestStruct*>()->value, 42);

    // Check rvalue
    EXPECT_TRUE(v4.CanGet<TestStruct&&>());
    EXPECT_EQ(v4.Get<TestStruct&&>().value, 42);

    // Modify original and check pointer reflects change
    obj.value = 100;
    EXPECT_EQ(v2.Get<TestStruct&>().value, 100);
    EXPECT_EQ(v3.Get<TestStruct*>()->value, 100);
}
}
